...
@pytest.mark.parametrize("raw", [
    ...
    decision(target_visible="false"), decision(target_visible=None), decision(target_visible=0),
    decision(None, {}, target_visible=False, goal_complete=True),
    json.dumps({key: value for key, value in json.loads(decision()).items() if key != "target_visible"}),
])
...
def test_unseen_target_scans_once_without_reversing_and_checks_final_frame(setup_loop, monkeypatch, caplog):
    controller, camera, model, memory = setup_loop
    speech = Mock()
    monkeypatch.setattr(loop, "speak", speech)
    # The model's oscillating proposals never control scan direction.
    model.side_effect = [decision(args={"degrees": degrees}, target_visible=False)
                         for degrees in [-30, 30, -60, 45, -90, 20]] + [decision(None, {}, target_visible=False)]
    state = RobotState(current_goal="find the red chair")

    async def run():
        for step in range(1, 7):
            await loop.run_episode(state, controller)
            assert state.search_active and state.search_direction == 1
            assert state.search_rotation_deg == step * 60
            assert controller.turn.call_count == camera.call_count == model.await_count == step
            assert state.finished_goal is None
            context = model.call_args.args[1]
            assert context["rotational_search"]
            assert context["search_rotation_deg"] == (step - 1) * 60
        speech.assert_not_called()  # 360 reached, but final view has not been inspected yet.
        await loop.run_episode(state, controller)
        await loop.run_episode(state, controller)  # Exhausted scan cannot restart itself.

    with caplog.at_level(logging.INFO):
        asyncio.run(run())
    assert camera.call_count == model.await_count == 7
    assert [c.kwargs["degrees"] for c in controller.turn.call_args_list] == [60] * 6
    assert len([c for c in controller.mock_calls if c[0] in PHYSICAL_ACTIONS]) == 6
    assert not state.search_active and state.search_direction == 0 and state.search_rotation_deg == 0
    assert state.finished_goal == state.current_goal and state.search_goal is None
    speech.assert_called_once_with("Target not found after a full 360-degree scan.")
    assert "Search started:" in caplog.text and "direction=left" in caplog.text
    assert "cumulative_deg=360.0/360" in caplog.text and "full 360 scan completed" in caplog.text
    assert memory.enqueue_memory.call_args.args[0] == "mission_summary"
    assert json.loads(memory.enqueue_memory.call_args.args[1])["goal_complete"] is False


@pytest.mark.parametrize("increment,expected", [(70, [70] * 5 + [10]), (120, [90] * 4)])
def test_search_increment_respects_tool_bound_and_remaining_angle(increment, expected, setup_loop, monkeypatch):
    controller, _, model, _ = setup_loop
    model.return_value = decision(None, {}, target_visible=False)
    monkeypatch.setattr(loop, "SEARCH_TURN_DEG", increment)
    state = RobotState(current_goal="search the room for a person")
    async def run():
        for _ in expected:
            await loop.run_episode(state, controller)
    asyncio.run(run())
    assert [c.kwargs["degrees"] for c in controller.turn.call_args_list] == expected
    assert state.search_rotation_deg == 360


@pytest.mark.parametrize("turns_before_detection", [2, 6])
def test_detection_resets_scan_and_resumes_normal_action_even_on_final_view(turns_before_detection, setup_loop, caplog):
    controller, _, model, _ = setup_loop
    model.side_effect = [decision(None, {}, target_visible=False)] * turns_before_detection + [
        decision("forward", {"distance_m": 0.3}, target_visible=True)]
    state = RobotState(current_goal="locate the red chair")
    async def run():
        for _ in range(turns_before_detection + 1):
            await loop.run_episode(state, controller)
    with caplog.at_level(logging.INFO):
        asyncio.run(run())
    assert controller.turn.call_count == turns_before_detection
    controller.forward.assert_called_once_with(0.3)
    assert not state.search_active and state.search_rotation_deg == state.search_direction == 0
    assert state.finished_goal is None and state.search_goal is None
    assert "Search target detected:" in caplog.text and "target not found" not in caplog.text


@pytest.mark.parametrize("failure", ["model", "malformed", "hardware", "stale_frame"])
def test_search_failure_never_advances_or_resets_scan(failure, setup_loop):
    controller, _, model, _ = setup_loop
    model.return_value = decision(None, {}, target_visible=False)
    state = RobotState(current_goal="look for the red chair")
    async def run():
        await loop.run_episode(state, controller)
        if failure == "model":
            model.side_effect = RuntimeError("temporarily unavailable")
        elif failure == "malformed":
            model.return_value = decision(target_visible="false")
        elif failure == "hardware":
            controller.turn.side_effect = RoboMasterError("action failed")
        else:
            controller.get_camera_state.side_effect = None
            controller.get_camera_state.return_value = {"last_frame_monotonic_s": state.last_frame_at}
        await loop.run_episode(state, controller)
    asyncio.run(run())
    assert state.search_active and state.search_direction == 1 and state.search_rotation_deg == 60
    assert controller.turn.call_count == (2 if failure == "hardware" else 1)
    assert state.finished_goal is None and not state.scene_fresh
    assert state.retry_at > time.monotonic()


def test_search_honors_stop_and_resets_for_a_different_goal(setup_loop):
    controller, _, model, _ = setup_loop
    state = RobotState(current_goal="find the red chair")
    async def run():
        model.return_value = decision(None, {}, target_visible=False)
        await loop.run_episode(state, controller)
        model.return_value = decision("stop", {}, target_visible=False)
        await loop.run_episode(state, controller)
        controller.stop.assert_called_once()
        assert state.search_rotation_deg == 60 and controller.turn.call_count == 1
        state.current_goal = "Please search for the blue chair"
        model.return_value = decision(None, {}, target_visible=False)
        await loop.run_episode(state, controller)
        assert state.search_rotation_deg == 60 and state.search_goal == state.current_goal
        assert model.call_args.args[1]["search_rotation_deg"] == 0
        state.current_goal = "inspect the doorway"
        model.return_value = decision(args={"degrees": -30}, target_visible=False)
        await loop.run_episode(state, controller)
    asyncio.run(run())
    controller.turn.assert_called_with(degrees=-30)
    assert not state.search_active and state.search_rotation_deg == 0
    assert not model.call_args.args[1]["rotational_search"]


@pytest.mark.parametrize("increment", [0, -60, float("nan"), float("inf")])
def test_invalid_search_increment_never_moves(increment, setup_loop, monkeypatch):
    controller, _, model, _ = setup_loop
    model.return_value = decision(None, {}, target_visible=False)
    monkeypatch.setattr(loop, "SEARCH_TURN_DEG", increment)
    state = asyncio.run(loop.run_episode(RobotState(current_goal="find chair"), controller))
    controller.turn.assert_not_called()
    assert not state.search_active and state.retry_at > time.monotonic()
