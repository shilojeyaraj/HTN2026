"""End-to-end FFmpeg A/V check with synthetic inputs and silent playback."""
...
@unittest.skipUnless(shutil.which("ffmpeg") and shutil.which("ffplay"), "requires ffmpeg/ffplay")
class WebcamAudioTest(unittest.TestCase):
    def test_video_audio_playback_and_cleanup(self):
        real_popen = subprocess.Popen
        frames, audio, results, errors, processes = [], [], [], [], []

        def capture_process(command, **kwargs):
            # Replace only physical input devices; use real encoders, pipes,
            # TCP framing, receiver, and ffplay with SDL's silent output driver.
        ...
        def serve():
            try:
                with server, audio_playback() as play:
        ...
        with patch.dict(os.environ, {"SDL_AUDIODRIVER": "dummy"}), \
                patch("pi.client.subprocess.Popen", side_effect=capture_process):
            worker.start()
            try:
        ...
        self.assertFalse(leaked, "capture/player process leaked")
