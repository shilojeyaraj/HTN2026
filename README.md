...
Planner arguments are validated before execution: distances are clamped to 0.05–0.75 m, turns to ±90°, arm deltas to ±80 mm per axis, and speech to 240 characters. Missing/invalid arguments are logged and rejected without moving. Each response must contain one observation/target_visible/target_alignment/tool/args/goal_complete/finding object, with an optional boolean search_active. The entire response is validated before execution; malformed responses are logged verbatim and cause no movement. Each fresh frame authorizes at most one action. After a physical action, the next accepted frame must have arrived after that action finished. Failed camera/model reads block movement, and goal_complete=true requires a null tool and terminates the mission. Terminal logs include requested and clamped arguments, chassis action and xy/z speeds, duration, and rejection details.

Target centering accepts normalized horizontal positions 0.40–0.60. Once aligned, it preserves heading through drift within 0.35–0.65. The planner receives this local alignment state and selects the next safe task step when centered enough. Turns explicitly marked as target-centering corrections are suppressed in that range; outside it, their magnitude is capped at `60 * abs(0.5 - center_x)` degrees, preserving smaller planner requests. Search and obstacle-avoidance turns keep their normal bounds. Tune `TARGET_CENTER_BAND`, `TARGET_ALIGNMENT_HOLD_BAND`, and `TARGET_ALIGNMENT_GAIN_DEG` in `brain/loop.py` for the camera mount. Logs report position category, deadband membership, hysteresis suppression, and correction angle. A suppressed turn never becomes an automatic forward drive; the next fresh-frame decision must authorize that approach.

Once the rover moves with a visible mission target, `target_tracking` remembers the approach. If a subsequent fresh frame reports that target absent, it stops, clears alignment, and restarts the LOW/HIGH scan in +60° increments from its current heading. The existing world model and heading estimates are preserved. Reacquisition interrupts the scan immediately and uses that same image decision for the next safe action. If a full scan finds nothing, visual search continues, but autonomous translations and goal completion remain blocked until the target reappears. Detection happens between bounded movement steps, after the SDK action completes; it does not interrupt an in-flight SDK move.

For ordinary planner-directed searches, search history guides the planner without overriding its actions. `RobotState.search_active`, `search_direction` (+1 left / -1 right), and `search_rotation_deg` track completed, clamped search turns. Each decision also receives `relative_heading_deg` and the last 36 `inspected_viewpoints`, including pose, observation, and target visibility before each action. Headings are command-based estimates relative to the goal's starting view, not compass telemetry; a failed turn makes the estimate unknown. Rotation totals include revisited arcs and do not prove coverage. The planner should continue a useful search direction, but can adjust the camera, reposition, or reverse when evidence warrants it. Outside the startup and reacquisition scans there is no forced 60° turn or automatic termination at 360°. Detection ends active searching while preserving view history; a new goal resets that history. Completion is model-selected and stops further actions. DIRECT commands retain local decomposition without perception; direct turns update the heading estimate without claiming an inspected view.
...
Viewpoint guidance treats floor/ceiling and clipped objects as task-dependent clues, not automatic arm commands. Some visible floor is normal; a low view can help with a gap. The planner compares fresh views with `last_camera_adjustment` (bounded arguments, outcome, and pre-adjustment observation when available), which persists beyond recent action history and includes manual adjustments and failures. Height/orientation are qualitative visual judgments, not measured camera telemetry. During traversal, disappearing furniture or doorway geometry is not proof of completion; the planner uses movement history and the forward view to assess clearance and restores a useful view if needed before finishing. Every camera adjustment still requires fresh perception before another autonomous physical action.
