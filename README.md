...
`turn 360` becomes four `turn(90)` calls; `move forward 1.5 m` becomes two `forward(0.75)` calls. Signed turns use positive = left and negative = right. Numeric translations require metres (`m`, `meters`, or `metres`). “Slightly” and “a little” use `SLIGHT_TRANSLATION_M=0.2` and `SLIGHT_TURN_DEG=15` in `brain/direct_commands.py`. Gripper open/close, arm recenter, and explicit arm moves such as “move arm up 30 mm” also route directly. A spoken direct command overrides the current cycle; “stop” ends the active mission.
