...
The planner can call bounded `forward`, `backward`, `strafe_left`, `strafe_right`, `turn`, and `stop` tools. Motion uses `ep.chassis.move(...).wait_for_completed()`; `stop` uses `ep.chassis.drive_speed(x=0, y=0, z=0)`.
