...
Planner arguments are validated before execution: distances are clamped to 0.05–0.75 m, turns to ±90°, arm deltas to ±80 mm per axis, and speech to 240 characters. Missing/invalid arguments are logged and rejected without moving. Each response must contain exactly one observation/target_visible/tool/args/goal_complete/finding object. The entire response is validated before execution; malformed responses are logged verbatim and cause no movement. Each fresh frame authorizes at most one action. After a physical action, the next accepted frame must have arrived after that action finished. Failed camera/model reads block movement, and goal_complete=true requires a null tool and terminates the mission. Terminal logs include requested and clamped arguments, chassis action and xy/z speeds, duration, and rejection details.

Goals beginning with `find`, `search`, `locate`, or `look for` (optionally preceded by `please`) use deterministic rotational search while `target_visible=false`. The scan chooses left once and turns by `SEARCH_TURN_DEG=60.0` in `brain/loop.py`, subject to the existing ±90° tool bound and the remaining angle. Every step requires fresh perception. `RobotState.search_active`, `search_direction` (+1 left / -1 right), and `search_rotation_deg` track completed commanded turns, not measured angular displacement. Model suggestions cannot reverse an active scan; a safety `stop` pauses it. Detection resets the scan and resumes normal centering/approach. After 360°, one final fresh frame is inspected; if still unseen, the robot reports not found, resets search state, and ends the mission. A changed goal also resets the scan. DIRECT commands such as `turn 360` retain local decomposition without perception.
