...
- **DIRECT:** exact motion commands are parsed once, split into bounded local tool calls, executed once, and the application exits. There are no camera or cloud inference calls for these commands. Every chunk still passes through the existing argument validator and RoboMaster controller. A failed chunk stops the maneuver; Ctrl+C requests a stop and closes the connection.
...
Both vision and planning use `gpt-4.1`. Vision sends the JPEG directly to the OpenAI Responses API and requires `OPENAI_API_KEY` in `.env`. Planning and memory stay on Backboard, using `BACKBOARD_API_KEY` and the `openai/gpt-4.1` route. The vision HTTP client is reused across frames and closed at shutdown. See the [OpenAI vision input format](https://developers.openai.com/api/docs/guides/images-vision#analyze-images).

Automatic model switching is disabled by default (`BACKBOARD_FALLBACK_MODEL=`); there is no Gemini or mini-model fallback. Model/provider overrides are in `.env.example`. Temporary planner limits honor provider retry delays (including HTTP `Retry-After`) and retry up to three times using GPT-4.1. These retries keep the current scene, goal, thread, and tool results, without executing actions or starting another episode. Accepted tool results are never submitted again after an embedded provider error. If planner recovery is exhausted or vision quota/billing is unavailable, the mission stops with an actionable error and closes the robot connection.

Vision requests start at least `VISION_MIN_INTERVAL_S=6` seconds apart, with additional cooldown after rate limits. Failed vision skips planning and movement; the next attempt captures a new frame. Backboard receives only the textual scene description. Missing or unauthorized OpenAI credentials stop the mission instead of retrying indefinitely. Existing Gemini keys are no longer used by the live loop.
